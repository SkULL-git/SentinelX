...
import { UploadCloud, FileText, CheckCircle2, Play, RefreshCw, Activity, ArrowRight, ShieldCheck } from 'lucide-react';
...
export const TrafficAnalysis = ({
  ...
  const handleFileChange = (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setUploadStatus(`Loaded custom file: ${file.name} (${(file.size / 1024).toFixed(1)} KB)`);
    }
  };

  const handleDemoClick = () => {
    onLoadScenario();
    setSelectedFileName('demo_enterprise_attack.pcap');
    setUploadStatus('Loaded representative demo attack PCAP file (24,820 packets).');
  };
    ...
    <div className="space-y-6">
      ...
      <div className="pb-4 border-b border-slate-800 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Ingest raw packet captures (PCAP) or IPFIX flow CSV telemetry to synthesize continuous temporal network states.
          </p>
        </div>
      ...
      <div className="p-3.5 rounded-lg bg-cyan-950/30 border border-cyan-500/30 flex items-center justify-between text-xs text-cyan-200">
        <div className="flex items-center space-x-2">
          ...
          <span>
            <strong>Demo Mode</strong> — simulated analysis using representative network telemetry.
          </span>
        </div>
      ...
      <div className="soc-card p-6 border border-dashed border-slate-700 bg-slate-950/40 text-center hover:border-cyan-500/50 transition">
        <div className="max-w-md mx-auto space-y-3">
          ...
          <p className="text-xs text-slate-400">
            Drag and drop network capture files or select a local dataset. Supported formats: <span className="font-mono text-cyan-300 font-semibold">.pcap, .pcapng, .csv</span>
          </p>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
            ...
            >
              ...
              <span>Load Demo Attack Scenario</span>
            </button>
          </div>

          {uploadStatus && (
            <div className="mt-3 p-2.5 rounded bg-emerald-950/40 border border-emerald-800 text-xs font-mono text-emerald-300 flex items-center justify-center space-x-2">
